...
class Mujoco_Dset(object):
    def __init__(self, expert_path, train_fraction=0.7, traj_limitation=-1, randomize=True):
        ...
        self.rets = traj_data['returns'][:traj_limitation]
...
def test(expert_path, traj_limitation, plot):
    dset = Mujoco_Dset(expert_path, traj_limitation=-1)
    print(np.mean(dset.rets), np.std(dset.rets))
    if plot:
        dset.plot()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--expert_path", type=str, default="data/export_demonstrations_Hopper-v2_model_2310_mean_2004.npz")
